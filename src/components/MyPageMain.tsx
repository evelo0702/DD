...
export default function Main({
  ...
  const handleSaveCategory = async (username: string, data: CategoryData[]) => {
    let result = await updateUserCategory(data, username);
    console.log(result);
    await refetch();
  };
  useEffect(() => {
  ...
  const updateFolderName = async (title: string, newTitle: string) => {
    ...
    let res = await updateUserFolders(title, data.username, "change", newTitle);
    await refetch();
    console.log(res);
  };
  ...
  return (
    <div className="w-full h-full  p-7 border-4 border-brown-50 md:rounded-e-lg rounded-b-lg bg-white shadow-lg ">
      {isActive === "user" && (
        <div className="grid gap-6">
          ...
          {/* 유저 정보 섹션 */}
          <div className="p-5 bg-gray-50 border rounded-lg shadow-md">
            {data && (
      ...
      {isActive === "like" && (
        <div>
          <div className="p-5 bg-gray-50 border rounded-lg shadow-md">
            <CardList totaldata={likeData} folder={folder} />
          </div>
        </div>
      )}
      {isActive === "save" && saveData && (
        <div>
          ...
          <div className="p-5 bg-gray-50 border rounded-lg shadow-md">
            <CardList totaldata={saveData.savedPosts} folder={folder} />
          </div>
        </div>
      )}
    </div>
